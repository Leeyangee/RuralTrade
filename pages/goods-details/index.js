...
import leeya_bug from '../../leeya_bug/leeya_bug.js'
...
Page({
  ...
  async getGoodsDetailAndKanjieInfo(goodsId) {
    ...
    const that = this;
    console.log(await WXAPI.goodsDetail(goodsId, token ? token : ''))
    const goodsDetailRes = leeya_bug.getOurGoodsDetailsBy(leeya_bug.getOurOpenId(),goodsId)
  ...
  async shopSubdetail(shopId) {
    ...
    const res = leeya_bug.getOurShopDetailsBy(leeya_bug.getOurOpenId(),shopId);
    if (res.code == 0) {
      this.setData({
