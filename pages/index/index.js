...
import catogoriesController from '../../leeya_bug/controllers/catogoriesController'
...
Page({
  ...
  onLoad: function(e) {
    ...
    this.initBanners()
    catogoriesController.getCatogories(this)
    this.cmsCategories()
    ...
    WXAPI.goodsv2({
      ...
      if (res.code === 0){
        ...
        that.setData({
          goodsRecommend: leeya_bug.getOurBaoGoodsListByUserId()
        })
      }      
    })
    */
    that.setData({
      goodsRecommend: leeya_bug.getOurBaoGoodsListByUserId(leeya_bug.getOurOpenId())
    })
    //that.getCoupons()
    //that.getNotice()
    //that.kanjiaGoods()
  ...
  async initBanners(){
    const _data = {}
    // 读取头部轮播图
   const res1 = leeya_bug.getOurBanners(leeya_bug.getOurOpenId(),'index');
    if (res1.code == 700) {
      wx.showModal({
        title: '提示',
  ...
  async goodsDynamic(){
    const res = leeya_bug.getOurGoodsDynamic(leeya_bug.getOurOpenId())
    if (res.code == 0) {
      this.setData({
  ...
  async categories(categories){
    //wx.getStorageSync('openId')    用户openId
    //wx.getStorageSync('avatarUrl') 微信头像
    //wx.getStorageSync('nickName')  微信昵称
    //dianhua
    //xinming

    //leeya_bug.getOurGoodsByCategorie(1);
    this.setData({
  ...
  async getGoodsList(categoryId, append) {
    ...
    */
    wx.hideLoading()
    let goods = [];
    goods = leeya_bug.getOurGoodsListByUserId(leeya_bug.getOurOpenId());
    this.setData({
      loadingMoreHidden: true,
      goods: goods,
    });
